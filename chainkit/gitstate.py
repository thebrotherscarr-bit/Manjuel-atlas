...
the court may rule; across the wall, only he lands." So chainkit reads the
...
def head_only(ground: Path) -> GitState:
    """HEAD, branch and subject -- and NEVER `git status`.

    `git status` refreshes the index and, from a sandboxed mount, leaves
    `.git/index.lock` the mount cannot remove (CLAUDE.md's first trap).
    The hands ledger (seatlog.hand_open/hand_close) is written BY hands,
    often from a sandbox -- and its first close, 2026-09-08 12:56, called
    read() and left exactly that lock. A hand's line needs the commit it
    stood on, not the dirty count; `dirty` is reported as None (unasked).
    """
    ground = Path(ground)
    try:
        rc, out = _run(["rev-parse", "--is-inside-work-tree"], ground)
...
def suggest_commit(ground: Path, message: str) -> str:
    ...
    safe = message.replace('"', "'").strip() or "chainkit: sitting"
...
# fetched it. Those are OFF unless CHAINKIT_GIT_REMOTE=1 is set in the
# environment, so a model cannot reach a remote by deciding to.
# =====================================================================

import os

REMOTE_ENV = "CHAINKIT_GIT_REMOTE"
...
def _write(args: list[str], ground: Path) -> tuple[int, str, str]:
    try:
        p = subprocess.run(
            ["git", *args], cwd=str(ground), capture_output=True, text=True, timeout=60
        )
