...
- READ-ONLY over source ground. Roots are read and never written. The index
  lives in chainkit's own ground (LAW 2: originals are read-only; packets
...
def _too_old_to_index(path) -> bool:
    ...
    Set CHAINKIT_LOG_HORIZON_DAYS=0 to index every transcript ever written.
    ...
    try:
        days = float(_os.environ.get("CHAINKIT_LOG_HORIZON_DAYS",
                                     LOG_HORIZON_DAYS))
    except ValueError:
...
def load_roots(config: Path, default: list[Path], base: Path | None = None) -> list[Path]:
    ...
    Relative entries resolve against `base` (chainkit's own ground), NOT the
