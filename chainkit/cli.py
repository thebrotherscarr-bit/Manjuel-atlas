...
def _cmd_warm(sess: Session, quiet: bool = False) -> None:
    ...
    # and charge THEM the reload -- not a cost chainkit gets to spend quietly.
    ...
    if others:
        ...
        if need > free:
            ...
            print("  /models shows the split. CHAINKIT_VRAM_GB sets the budget.\n")
            return
...
def _cmd_models(sess: Session, arg: str = "") -> None:
    ...
    print()
    print(f"  chainkit SETS per request:   keep_alive={KEEP_ALIVE}, num_ctx and")
    print( "                               num_predict from each seat's markdown.")
    print( "  chainkit CANNOT set these -- the ollama SERVER reads them at its own")
...
def _cmd_git(sess: Session) -> None:
    ...
    if not g.is_repo and not g.error:
        ...
        print(f'    git -C "{ROOT}" init && git -C "{ROOT}" add -A && '
              f'git -C "{ROOT}" commit -m "chainkit: initial"')
    elif g.is_repo and g.dirty:
        print("  To version this sitting, run this YOURSELF:")
        print("    " + gitstate.suggest_commit(ROOT, f"chainkit: sitting {sess.sitting.n}"))
        print("  chainkit never commits (LAW 6: the gate is final).")
    print()
...
def _cmd_toll(sess: Session, attended: bool = True) -> None:
    ...
    if g.is_repo and g.dirty:
        ...
        print("    " + gitstate.suggest_commit(ROOT, f"chainkit: sitting {st.n}"))
    print()
...
def main() -> int:
    ...
    if os.environ.get("CHAINKIT_NO_WARM", "").strip() not in ("1", "true", "yes"):
        # chainkit does NOT start Ollama -- it connects to the one already
    ...
    try:
        ...
        # chain.py, which catches only KeyboardInterrupt: the watcher thread
