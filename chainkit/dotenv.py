...
---------------------------
Anything chainkit itself reads, because chainkit reads it at ITS startup:

    CHAINKIT_KEEP_ALIVE=30m      how long Ollama holds a model (per-request)
    CHAINKIT_NO_WARM=1           skip warming models on launch
    CHAINKIT_GIT_REMOTE=1        allow git pull/push
    CHAINKIT_OLLAMA_HOST=...     where Ollama lives

What it CANNOT change
---------------------
`OLLAMA_NUM_PARALLEL` and `OLLAMA_MAX_LOADED_MODELS` are read by the `ollama
serve` PROCESS when IT starts. Setting them here puts them in chainkit's
...
appear here chainkit says plainly that they were ignored and where they belong.
