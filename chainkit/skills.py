...
# way out. Overridable with CHAINKIT_SKILL_TIMEOUT for a legitimately slow
# ground (index_ground over thousands of documents).
try:
    SKILL_TIMEOUT = float(os.environ.get("CHAINKIT_SKILL_TIMEOUT") or 300)
except ValueError:
    SKILL_TIMEOUT = 300.0
...
class SkillExecutionEnv:
    ...
    ground: Path = None      # chainkit's own dir: index + config live here
    ...
    for name, jail in spec.path_args:
        ...
        if (len(raw.split()) >= 3 and "/" not in raw and "\\" not in raw
            ...
            return (f"Refused: `{name}` must be a PATH, and {raw!r} is a "
                    ...
                    f"`agents`, `chainkit/intent.py`, `pipelines.md` — or "
...
def _commit_subject(env: SkillExecutionEnv, args: dict) -> str:
    ...
    # refuses "fix the greeting dispatch" over chainkit/intent.py exactly
...
def _proved(env: SkillExecutionEnv, args: dict) -> str:
    ...
    and the manifest report by chainkit.us. No count is stored in a prompt
    ...
    touched, newer = 0.0, []
    for d in ("chainkit", "agents", "skills", "tests"):
...
RACK_PULL_ENV = "CHAINKIT_RACK_PULL"
...
def _semantic_search(env: SkillExecutionEnv, args: dict) -> str:
    ...
    def _rank(h):
        ...
        if "/foundation/" in path or path.endswith("memory.md"):
            ...
            bonus = 0.03
        elif "/chainkit/" in path or "/tests/" in path:
            bonus = -0.03
    ...
    def execute(self, action: str, args: dict, env: SkillExecutionEnv) -> str:
        ...
        try:
            ...
            return (f"Refused: '{action}' did not finish within "
                    ...
                    f"bound with CHAINKIT_SKILL_TIMEOUT if this skill is "
                    f"legitimately slow.")
        except Exception as exc:
