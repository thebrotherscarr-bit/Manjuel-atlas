...
def suite_tally(ROOT: Path) -> str:
    ...
    if not p.exists():
        return "not run here yet   python tests/test_chainkit.py"
    try:
...
def brief_facts(sess, ROOT: Path, git=None) -> list[str]:
    ...
    # operator sees at once whether the hand read 94 or 88.
    try:
        out.append(_seatlog.hands_line(ROOT))
    except Exception as exc:
        out.append(f"    hands    unreadable ({exc})")
