...
def reconcile(ground, registry, library, installed: set | None = None
    ...
    seat_recs = {r["id"][len("chain_"):] for r in records
                 if r.get("kind") == "agent" and r["id"].startswith("chain_")}
    ...
    for a in sorted(seat_recs - seats_on_disk):
        findings.append(Finding("DRIFT", f"us/chain_{a}", "record", "a seat",
    ...
    for name in sorted(seats_on_disk & seat_recs):
        ...
        if set(may) & WRITING_SKILLS and not allows_edit:
            findings.append(Finding("DRIFT", f"us/chain_{name}", "permission.edit",
