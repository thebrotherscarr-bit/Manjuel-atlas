...
try:
    TURN_DEADLINE = float(os.environ.get("CHAINKIT_TURN_DEADLINE") or 600)
except ValueError:
    TURN_DEADLINE = 600.0


# A skill-shaped word: letters_with_underscores standing alone -- not a
# file name (`run_history.jsonl`, `test_chainkit.py`) and not a path part.
...
def recompose(ctx: RunContext, report=print) -> bool:
    ...
    if late:
        ...
        lines.append("What is above is the seats that sat. Ask again for the "
                     "rest, or raise CHAINKIT_TURN_DEADLINE.")
        blocks.append("\n".join(lines))
    block = "\n\n".join(blocks)
