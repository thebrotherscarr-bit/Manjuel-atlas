...
- RULED 2026-09-09 ("sounds perfect, actually. that's what i wanted, thats what the chat/router gating is for") — PHRASES FOR THE DOOR, KEYWORDS FOR THE ROUTER. OPEN — not built: the door is still handed the bare list of skill keywords in its prompt, which is the provocation llama 3.x answers with a tool call. The door is Steward on `llama3.2` (default: Steward → Router when needs_tool → Steward when worked), so the seat being provoked is the one that greets and the one that closes. The decision is made; the change is not.
...
- RULED 2026-09-09 ("4.3 facts only. yep, sounds good") — `rack_report` gives FACTS ONLY unless a judgement is asked for. OPEN — not built: it still hands the Router a model's reading beneath the facts, and the Router summarises the reading (sitting 85, three times). The decision is made; the change is not.
