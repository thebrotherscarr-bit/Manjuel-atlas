"""chainkit prove — the suite, run with no model and no network.
...
# `python tests/test_chainkit.py intent` runs only the strokes whose
...
from chainkit import mathkit as M                                    # noqa: E402
from chainkit import memory as MEM                                   # noqa: E402
from chainkit import gitstate, seatlog, transcript                   # noqa: E402
from chainkit.context import RunContext, StepResult                              # noqa: E402
from chainkit.drift import DriftChecker                              # noqa: E402
from chainkit.pipeline import (MAX_TOOL_STEPS, Refused, read_flags,  # noqa: E402
                               read_verdict, run_pipeline)
from chainkit.registry import AgentRegistry, PipelineBook, RegistryError  # noqa: E402
from chainkit.skills import SkillExecutionEnv, SkillLibrary, extract_tool_call  # noqa: E402
from chainkit.vectors import IndexError_, VectorIndex, chunk_text    # noqa: E402
from chainkit import vram                                            # noqa: E402
from chainkit import seating                                         # noqa: E402
...
def prompted_steward(reg):
    ...
    from chainkit.registry import Agent
...
def test_step_conditions(reg, lib, book):
    from chainkit.registry import PipelineBook
...
def test_evaluator_isolation(reg, lib, book):
    ...
    from chainkit.pipeline import build_prompt
    g = Path(tempfile.mkdtemp())
    r = Stub()
...
def test_session4_regressions(reg, lib, book):
    ...
    from chainkit.drift import MIN_SOURCE_CHARS, MIN_OUTPUT_CHARS
...
def test_dotenv():
    ...
    from chainkit import dotenv
    g = Path(tempfile.mkdtemp())
    (g / ".env").write_text(
        "# a comment\n"
        "CHAINKIT_TEST_NEW=abc\n"
        "export CHAINKIT_TEST_QUOTED='shh'\n"
        "CHAINKIT_TEST_TAKEN=fromfile\n"
        "OLLAMA_NUM_PARALLEL=4\n"
        "OLLAMA_MAX_LOADED_MODELS=3\n", encoding="utf-8")

    os.environ.pop("CHAINKIT_TEST_NEW", None)
    os.environ.pop("CHAINKIT_TEST_QUOTED", None)
    os.environ["CHAINKIT_TEST_TAKEN"] = "fromshell"
    os.environ.pop("OLLAMA_NUM_PARALLEL", None)

    applied, already, server = dotenv.load(g / ".env")

    check("a .env sets what is missing", "CHAINKIT_TEST_NEW" in applied
          and os.environ["CHAINKIT_TEST_NEW"] == "abc")
    check("quotes and `export` are handled",
          os.environ.get("CHAINKIT_TEST_QUOTED") == "shh")
    check("the shell wins over the file",
          os.environ["CHAINKIT_TEST_TAKEN"] == "fromshell"
          and "CHAINKIT_TEST_TAKEN" in already)
    ...
    check("the report names keys but never values",
          "CHAINKIT_TEST_NEW" in lines and "abc" not in lines and "shh" not in lines)
    check("and says where the server vars actually belong",
          "does nothing" in lines and "restart it" in lines)

    for k in ("CHAINKIT_TEST_NEW", "CHAINKIT_TEST_QUOTED", "CHAINKIT_TEST_TAKEN"):
...
def test_rack(reg, lib):
    ...
    os.environ.pop("CHAINKIT_RACK_PULL", None)
...
def test_rack_sync(reg, lib):
    ...
    from chainkit import rack as R
    g = Path(tempfile.mkdtemp())
...
def test_a_door_that_calls_a_tool_hands_it_to_the_router(reg, lib, book):
    ...
    from chainkit.pipeline import strip_control
    ...
    from chainkit.pipeline import _SCAFFOLD_RE
...
def test_sitting_87_the_thread_the_scaffold_and_the_mention(reg, lib, book):
    ...
    """
    from chainkit import intent
    from chainkit.pipeline import read_flags, _SCAFFOLD_RE
...
def test_the_claude_md_system_and_the_ruling_loop(reg, lib, book):
    ...
    from chainkit import lawgate, seatlog
    from chainkit.pipeline import (MAX_RULING_TURNS, carried_blocks, build_prompt,
                                   note_partial_read, unread_parts, recompose)
    from chainkit.runtime import SALVAGE_MARK, _salvage, OllamaRuntime
    import inspect
    ...
    check("the CLI builds the standing once at open and hands it on every turn",
          "standing=getattr(sess, \"standing\", \"\")" in (ROOT / "chainkit" / "cli.py").read_text(encoding="utf-8")
          and "self.standing = _log.standing_block(ROOT)" in (ROOT / "chainkit" / "cli.py").read_text(encoding="utf-8"))

    # ---- D. the partial-read stamp ---------------------------------
    from chainkit.skills import windowed
...
def test_the_seat_bound(reg, lib, book):
    ...
    import dataclasses
    import time as _t
    from chainkit import runtime as _rt
    from chainkit.runtime import OllamaRuntime, SeatTimeout, RuntimeError_
    from chainkit.registry import AgentRegistry

    check("the ceiling is the operator's 700s and a dial (CHAINKIT_SEAT_TIMEOUT)",
          _rt.SEAT_TIMEOUT == 700.0 or os.environ.get("CHAINKIT_SEAT_TIMEOUT"))
    ...
    check("a transport timeout is the same named refusal",
          ...
          and "CHAINKIT_SEAT_TIMEOUT" in msg, msg)
    ...
    (d / "steward.md").write_text(
        ...
        encoding="utf-8")
    from chainkit.registry import AgentRegistry as _AR
...
def test_the_turn_deadline(reg, lib, book):
    ...
    import shutil
    import time as _t
    from chainkit.pipeline import run_pipeline, TURN_DEADLINE, _within_deadline

    check("the turn deadline is the operator's 600s and a dial (CHAINKIT_TURN_DEADLINE)",
          TURN_DEADLINE == 600.0 or os.environ.get("CHAINKIT_TURN_DEADLINE"))
    ...
    from chainkit.pipeline import _sub_runner
...
def test_the_loops_of_2026_09_08(reg, lib, book):
    ...
    import threading
    import time as _t
    from chainkit import skills as _sk
    from chainkit.watch import GroundWatch
    ...
    src = (ROOT / "chainkit" / "skills.py").read_text(encoding="utf-8")
    ...
    cli = (ROOT / "chainkit" / "cli.py").read_text(encoding="utf-8")
...
def test_the_release_gate(reg, lib, book):
    ...
    g = Path(tempfile.mkdtemp())
    for d in ("chainkit", "agents", "skills", "tests", "sessions"):
        (g / d).mkdir()
    (g / "chainkit" / "a.py").write_text("x = 1\n", encoding="utf-8")
    ...
    check("no hands ledger yet passes and says it is 0.1.6's",
          _rel.hands(g).ok and "0.1.6" in _rel.hands(g).why)
    (g / "sessions" / "hands.jsonl").write_text(_json.dumps({"opened": "x"}) + "\n", encoding="utf-8")
    check("a hands ledger whose last line never closed is refused", not _rel.hands(g).ok)
    (g / "sessions" / "hands.jsonl").write_text(_json.dumps({"opened": "x", "closed": "y"}) + "\n",
                                                encoding="utf-8")
    check("... and passes when the last hand closed", _rel.hands(g).ok)
...
def test_the_p0_of_the_review(reg, lib, book):
    ...
    import shutil
    import time as _t
    from chainkit.pipeline import run_pipeline, recompose, decided_call
    from chainkit import transcript as _tr
    from chainkit import skills as _sk
    sys.path.insert(0, str(ROOT / "tests"))
    ...
    class Cut(Stub):
        def chat(self, agent, prompt, stream_to=None, tools=None, think_to=None, think=None):
            if agent.key == "jesster":
                from chainkit.runtime import SeatTimeout
    ...
    from chainkit.cli import _unsourced
...
def test_the_story_and_the_hands(reg, lib, book):
    ...
    from chainkit import intent as _in
    from chainkit import cli as _cli
    from chainkit.pipeline import carried_blocks, run_pipeline
    ...
    (h / "law").mkdir(); (h / "sessions").mkdir()
    (h / "CLAUDE.md").write_text("# rules\n", encoding="utf-8")
    (h / "law" / "SITTING_LAWS.md").write_text("# laws\n", encoding="utf-8")
    (h / "DAYBOOK.md").write_text("# D\n\n## Session 6 — today\n\n**Standing** — x\n", encoding="utf-8")
    (h / "HANDOFF.md").write_text("# H\n\n## HANDOFF FOR 2026-09-08 — read this\n\n## HANDOFF FOR 2026-09-07 — old\n",
                                  encoding="utf-8")
    check("no hand yet is said so", "none recorded" in _sl.hands_line(h))
    opened = _sl.hand_open(h, hand="claude", note="0.1.6")
    check("the opening line fingerprints the rules AS READ and names what was read",
          opened["read"]["CLAUDE.md"] and opened["read"]["law/SITTING_LAWS.md"]
          and opened["daybook"].startswith("## Session 6") and opened["handoff"].startswith("## HANDOFF FOR 2026-09-08")
          and opened["closed"] == "", repr(opened)[:300])
    check("an open hand is flagged in the brief's line, by id",
          "OPEN:" in _sl.hands_line(h) and opened["id"] in _sl.hands_line(h) and "!!" in _sl.hands_line(h))
    check("the hands ledger never runs git status (the lock of 2026-09-08 12:56)",
          "gitstate.read(" not in (ROOT / "chainkit" / "seatlog.py").read_text(encoding="utf-8")
          .split("def hand_open", 1)[1].split("def hands_line", 1)[0])
    check("the gate refuses a hand that never closed", not _rel_hands_ok(h))
    closed = _sl.hand_close(h, edited=["chainkit/seatlog.py", "tests/x.py"], strokes="1800/1800",
                            restart_required=True)
    check("the closing line supersedes with the same id and says what was done",
          closed["id"] == opened["id"] and closed["closed"] and closed["edited"] == ["chainkit/seatlog.py", "tests/x.py"]
          and closed["strokes"] == "1800/1800" and closed["restart_required"])
    line = _sl.hands_line(h)
    check("the brief's line says closed, how many files, the strokes, and RESTART REQUIRED",
          "closed" in line and "edited 2 files" in line and "1800/1800" in line and "RESTART REQUIRED" in line, line)
    check("... and the gate passes", _rel_hands_ok(h))
    check("closing twice is refused", refuses(lambda: _sl.hand_close(h), RuntimeError))
    raw = (h / "sessions" / "hands.jsonl").read_bytes()
    check("the ledger is append-only, two lines, CRLF like sessions.jsonl",
          raw.count(b"\n") == 2 and b"\r\n" in raw)
    (h / "sessions" / "hands.jsonl").write_bytes(raw + b"{not json\r\n")
    check("a corrupt line is NAMED in the brief, not swallowed", "corrupt" in _sl.hands_line(h))
    check("the hand's door is a command: python -m chainkit.seatlog hand-open / hand-close / hands",
          "hand-open" in (ROOT / "chainkit" / "seatlog.py").read_text(encoding="utf-8")
          and 'if __name__ == "__main__"' in (ROOT / "chainkit" / "seatlog.py").read_text(encoding="utf-8"))
    boot_src = (ROOT / "chainkit" / "boot.py").read_text(encoding="utf-8")
    check("the brief prints the last hand beside the last sitting", "hands_line(ROOT)" in boot_src)
...
def _rel_hands_ok(ground: Path) -> bool:
    sys.path.insert(0, str(ROOT / "tests"))
    import release as _rel
    return _rel.hands(ground).ok
...
def test_the_ground_flag(reg, lib, book):
    ...
    # ---- the doors carry it ------------------------------------------
    cli_src = (ROOT / "chainkit" / "cli.py").read_text(encoding="utf-8")
    serve_src = (ROOT / "chainkit" / "serve.py").read_text(encoding="utf-8")
    chain_src = (ROOT / "chain.py").read_text(encoding="utf-8")
    ...
    check("chain.py names the flag for both doors", "--ground" in chain_src)
    check("the flag is spelled once", cli_src.count('"--ground"') == 1 and "GROUND_FLAG" in serve_src)


def test_the_hands_close_says_what_it_did(reg, lib, book):
    """G4 (the operator, 2026-09-09: "make sure every step taken is recorded
    in the logs ... keep everything on record and usable by the next
    agent/operator"). Every close before this one wrote `0 file(s) edited`
    unless the hand remembered a flag -- five such lines were written on the
    morning of the ruling -- and a close that says nothing is a line the next
    hand cannot use. An unnamed close now READS THE DISK: mtimes since the
    open, no git and therefore no lock (CLAUDE.md's first trap, and the fault
    the first hand_close ever run committed). It stamps `named` or `observed`
    so an inference is never read as the hand's own claim (LAW 5)."""
    ...
    (g / "chainkit").mkdir()
    old = g / "already.md"
    old.write_text("before the hand\n", encoding="utf-8")
    past = _time.time() - 3600
    _os.utime(old, (past, past))

    line = _sl.hand_open(g, hand="stroke", note="G4")
    check("the open writes on a ground that is not a repository at all",
          line["id"].startswith("H") and _sl.hands_path(g).is_file())

    _time.sleep(1.05)                      # mtime resolution, not a race
    (g / "touched.md").write_text("after the open\n", encoding="utf-8")
    (g / "chainkit" / "moved.py").write_text("# after\n", encoding="utf-8")
    (g / "logs" / "run.md").write_text("a transcript\n", encoding="utf-8")
    (g / "sessions" / "noise.jsonl").write_text("{}\n", encoding="utf-8")

    closed = _sl.hand_close(g, strokes="1/1")
    seen = closed["edited"]
    check("an unnamed close OBSERVES the disk instead of recording nothing",
          closed["edited_by"] == "observed" and "touched.md" in seen
          and "chainkit/moved.py" in seen, str(seen))
    check("a file untouched since before the open is not claimed",
          "already.md" not in seen, str(seen))
    check("derived trees are never attributed to a hand (logs/, sessions/)",
          not any(p.startswith(("logs/", "sessions/")) for p in seen), str(seen))
    check("NO GIT: nothing under .git is created on a non-repository ground",
          not (g / ".git").exists())

    _sl.hand_open(g, hand="stroke2")
    named = _sl.hand_close(g, edited=["b.md", "a.md", ""], strokes="2/2",
                           restart_required=True)
    check("a named --edited is believed, sorted, empties dropped, stamped `named`",
          named["edited"] == ["a.md", "b.md"] and named["edited_by"] == "named"
          and named["restart_required"] is True, str(named["edited"]))

    rows = [_json.loads(l) for l in
            _sl.hands_path(g).read_text(encoding="utf-8").splitlines() if l.strip()]
    check("every line is appended, never rewritten (LAW 1, LAW 8)",
          len(rows) == 4 and sum(1 for r in rows if r.get("closed")) == 2,
          f"{len(rows)} rows")
...
def test_the_headless_door(reg, lib, book):
    ...
    import json as _json
    import shutil
    from chainkit import serve as _sv
    from chainkit import seatlog as _sl
    from chainkit.runtime import RuntimeError_
    ...
    check("chain.py carries --headless and hands it to chainkit.serve",
          "--headless" in (ROOT / "chain.py").read_text(encoding="utf-8")
          and "chainkit.serve" in (ROOT / "chain.py").read_text(encoding="utf-8"))
    check("the door has a __main__ (python -m chainkit.serve)",
          'if __name__ == "__main__"' in (ROOT / "chainkit" / "serve.py").read_text(encoding="utf-8"))
    src_ = (ROOT / "chainkit" / "serve.py").read_text(encoding="utf-8")
    check("the door opens no socket, no daemon, no API (BUILDPATH's position kept)",
          not any(w in src_ for w in ("import socket", "http.server", "socketserver",
                                       "asyncio", "websocket")))
    engine = "".join((ROOT / "chainkit" / f).read_text(encoding="utf-8")
    ...
    check("the engine is not edited for it: pipeline, runtime, cli and skills know nothing of the door",
          not any(w in engine for w in ("import serve", "serve import", "serve.py", "chainkit.serve")))
...
def test_sitting_88_paths_and_evidence(reg, lib, book):
    ...
    """
    import shutil
    from chainkit.skills import unjail, find_by_name
    from chainkit.pipeline import build_prompt
    ...
    check("a real relative path is untouched", unjail("chainkit/intent.py") == "chainkit/intent.py")
    ...
    check("...and the read RAN on the real file",
          ...
          router_out[:200])
    from chainkit.pipeline import decided_call
...
def test_inspect_remember_that_and_the_brief(reg, lib, book):
    ...
    """
    import shutil
    from chainkit.skills import inspect_file, inspect_line, REVIEW_ONLY_SKILLS
    from chainkit import memory as mem, boot, seatlog
    from chainkit.cli import remember_cue
    ...
    src = (ROOT / "chainkit" / "cli.py").read_text(encoding="utf-8")
    ...
    check("...and nothing in the pipeline reads it (a seat's words never land memory)",
          "remember_cue" not in (ROOT / "chainkit" / "pipeline.py").read_text(encoding="utf-8"))
...
def test_thinking_models_are_not_swallowed(reg, lib, book):
    ...
    from chainkit.runtime import OllamaRuntime
...
def test_commit_subject_is_not_the_tool_name(reg, lib, book):
    ...
    from chainkit.skills import _commit_subject
    ...
    check("a real objective is used when content is empty",
          subj("", "commit the chainkit rebuild") == "commit the chainkit rebuild")
...
def test_seat_rack(reg, lib, book):
    ...
    from chainkit.registry import Step
    from chainkit.seating import Anchor, Seating, SeatingError, parse_anchor
...
def test_output_cannot_forge_the_record(reg, lib, book):
    ...
    from chainkit.transcript import quote_structure
...
def test_commit_subject_from_fact(reg, lib, book):
    ...
    #    (chain: HANDOFF.md, SEAT_LOG.md, chainkit/, sessions/)"
...
def test_spelling_is_conservative(reg, lib, book):
    ...
    check("nor a path",
          spelling.check("see chainkit/spelling.py").text == "see chainkit/spelling.py")
    check("nor a url",
...
def test_voice_degrades(reg, lib, book):
    ...
    check("transcription never reaches for a network",
          ...
          (ROOT / "chainkit" / "voice.py").read_text(encoding="utf-8"))
...
def test_parity_measures_without_deciding(reg, lib, book):
    ...
    check("nothing in parity reaches for the network",
          "antcli" not in (ROOT / "chainkit" / "parity.py").read_text(encoding="utf-8"))
...
def test_index_stays_in_research_and_off_the_keys(reg, lib, book):
    ...
    from chainkit.vectors import is_secret
...
def test_ollama_responses_read_in_both_shapes(reg, lib, book):
    ...
    """
    from chainkit.runtime import OllamaRuntime, _field
    from chainkit import vram as _vram
...
def test_the_estate_is_heard_correctly(reg, lib, book):
    ...
    from chainkit import voice, intent
    ...
    src = (ROOT / "chainkit" / "voice.py").read_text(encoding="utf-8")
...
def test_the_reasoner_tier(reg, lib, book):
    ...
    from chainkit.skills import _age_of
    import os as _os
    import time as _t
    stampf = g / "old.txt"
...
def test_coder_lands_files_and_review_wakes(reg, lib, book):
    ...
    from chainkit.pipeline import land_code
...
def test_the_landing_gate_parses_before_it_writes(reg, lib, book):
    ...
    happen."""
    from chainkit.pipeline import inspect_code, land_code, NETWORK_MODULES
...
def test_ground_eyes_are_live_readonly_and_jailed(reg, lib, book):
    ...
    check("the ground listing is live and names real folders",
          "chainkit/" in out and "agents/" in out, out[:80])
...
def test_the_card_is_priced_live(reg, lib, book):
    ...
    mid-question."""
    from chainkit import intent, vram as _vram
...
def test_drift_needs_a_source(reg, lib, book):
    ...
    from chainkit.drift import DriftChecker
...
def test_the_thread_sessions(reg, lib, book):
    ...
    from chainkit.cli import save_thread, load_thread
...
def test_the_estate_never_accuses_itself(reg, lib, book):
    ...
    """
    from chainkit import vram as _vram
    from chainkit.skills import EMBED_MODEL
...
def test_ollama_is_the_server(reg, lib, book):
    ...
    from chainkit.cli import Session
...
def test_the_table_has_eyes_not_hands(reg, lib, book):
    ...
    from chainkit.skills import REVIEW_ONLY_SKILLS
...
def test_the_claim_check(reg, lib, book):
    ...
    """
    from chainkit import intent
    from chainkit.context import StepResult
    from chainkit.skills import REVIEW_ONLY_SKILLS
    ...
    for said in (
            ...
            "run tests/test_chainkit.py and tell me the count",
            "nothing was found",
    ):
...
def test_attention_budget(reg, lib, book):
    ...
    from chainkit.context import select_dialogue
    from chainkit.pipeline import build_prompt
...
def test_recall_floor_and_timestamps(reg, lib, book):
    ...
    import time as _t
    from chainkit.context import select_dialogue, RECALL_FLOOR
...
def test_topic_boundaries(reg, lib, book):
    ...
    from chainkit import intent
    from chainkit.context import detect_shift, select_dialogue
    ...
    check("acknowledgments are HARNESS lines, never prompt text",
          ...
          (ROOT / "chainkit" / "pipeline.py").read_text(encoding="utf-8"))
...
def test_the_ground_watches_itself(reg, lib, book):
    ...
    from chainkit.watch import GroundWatch, available
...
def test_per_seat_voices(reg, lib, book):
    ...
    from chainkit import voice as V
    from chainkit.cli import _voice_for
    ...
    check("the voice rides the environment, never argv",
          "CHAINKIT_SPEAK_VOICE=(voice or \"\")" in src)
...
def test_client_data_is_shielded(reg, lib, book):
    ...
    from chainkit.vectors import is_protected
    from chainkit.watch import GroundWatch
...
def test_no_client_world_is_read_by_this_suite(reg, lib, book):
    ...
    src = (ROOT / "tests" / "test_chainkit.py").read_text(encoding="utf-8")
...
def test_the_review_can_send_work_back_once(reg, lib, book):
    ...
    from chainkit.pipeline import build_prompt
    from chainkit.seating import Seating, Step
...
def test_a_big_file_is_a_window_not_a_stump(reg, lib, book):
    ...
    from chainkit.skills import READ_WINDOW, windowed
...
def test_the_estate_can_reason_about_time(reg, lib, book):
    ...
    from chainkit import intent
    from chainkit.pipeline import build_prompt, now_block
    from chainkit.skills import RECENCY_WEIGHT, REVIEW_ONLY_SKILLS
...
def test_the_log_horizon(reg, lib, book):
    ...
    import os
    import time as _t
    from chainkit.vectors import LOG_HORIZON_DAYS, _too_old_to_index
    ...
    held = os.environ.pop("CHAINKIT_LOG_HORIZON_DAYS", None)
    try:
        os.environ["CHAINKIT_LOG_HORIZON_DAYS"] = "0"
        check("and the horizon can be turned off entirely",
              not _too_old_to_index(old_log))
        os.environ["CHAINKIT_LOG_HORIZON_DAYS"] = "not a number"
        check("a nonsense setting falls back to the default, never crashes",
              _too_old_to_index(old_log))
    finally:
        os.environ.pop("CHAINKIT_LOG_HORIZON_DAYS", None)
        if held is not None:
            os.environ["CHAINKIT_LOG_HORIZON_DAYS"] = held
...
def test_sitting70_regressions(reg, lib, book):
    ...
    """
    from chainkit import intent
    from chainkit.skills import WRITING_SKILLS
    ...
    from chainkit import voice as _v
    g = Path(tempfile.mkdtemp())
...
def test_the_shortlist_is_budget_not_judgement(reg, lib, book):
    ...
    from chainkit.pipeline import build_prompt
    from chainkit.skills import SHORTLIST_KEEP
...
def test_the_record_can_be_audited(reg, lib, book):
    ...
    (g / "index_roots.txt").write_text("chainkit\nworlds\n", encoding="utf-8")
...
def test_a_subtask_runs_scoped_and_bounded(reg, lib, book):
    ...
    from chainkit.pipeline import SUB_DEPTH_MAX, SUB_RUNS_MAX
...
def test_the_delivery_carries_what_actually_ran(reg, lib, book):
    ...
    from chainkit.pipeline import recompose
...
def test_a_skill_owns_its_own_dispatch(reg, lib, book):
    ...
    from chainkit import intent
    from chainkit.skills import (args_from_words, parse_says, parse_takes,
                                 SkillSpec)
...
def test_an_error_message_is_a_promise(reg, lib, book):
    ...
    from chainkit import intent
    from chainkit.skills import _wants_rebuild
...
def test_a_sitting_number_resolves_to_its_runs(reg, lib, book):
    ...
    (g / "SEAT_LOG.md").write_text(
        ...
        "**The seat:** chainkit REPL, session `S1`, 09:01-09:03.\n\n"
        "**WHAT RAN** (observed)\n\n"
        "  1. git status\n"
        "     default · 2 stages · 3.1s · logs/2026-09-01_090112_git_status.md\n\n"
        "## 2026-09-02 — sitting 63 — a sitting\n\n"
        "**The seat:** chainkit REPL, session `S2`, 10:49-11:01. Operator present.\n\n"
        "**WHAT RAN** (observed)\n\n"
...
def test_palette_commands_take_arguments_and_carry_methods(reg, lib, book):
    ...
    from chainkit.cli import ARGS_TOKEN, custom_commands, fill_args
    from chainkit.pipeline import build_prompt
...
def test_the_tool_loop_never_repeats_itself(reg, lib, book):
    ...
    from chainkit.pipeline import MAX_TOOL_STEPS
...
def test_the_boot_reports_what_was_proved(reg, lib, book):
    ...
    import json
    import time
    from chainkit.boot import suite_tally

    g = Path(tempfile.mkdtemp())
    (g / "tests").mkdir()
    for d in ("chainkit", "agents", "skills"):
        (g / d).mkdir()
    (g / "chainkit" / "x.py").write_text("# ground\n", encoding="utf-8")
    ...
    time.sleep(0.01)
    (g / "chainkit" / "x.py").write_text("# changed\n", encoding="utf-8")
    ...
    # NO `from test_chainkit import ...` HERE. begin_run and record_run are
...
def test_the_chain_writes_declared_newlines(reg, lib, book):
    ...
    for f in sorted((ROOT / "chainkit").glob("*.py")):
        if f.name == "voice.py":
            continue
...
def test_a_python_file_is_cut_by_definition_not_by_character(reg, lib, book):
    ...
    needed."""
    from chainkit.skills import windowed, READ_WINDOW
    ...
    m = windowed(src, "chainkit/skills.py")
    ...
    one = windowed(src, "chainkit/skills.py", "_commit_subject")
    ...
    ex = windowed(src, "chainkit/skills.py", "windowed")
    check("an EXACT name wins over a longer one containing it",
          "`windowed`," in ex, ex[:80])

    miss = windowed(src, "chainkit/skills.py", "no_such_function")
    ...
    check("a NUMBERED part of a .py is still a character window",
          "part 2 of" in windowed(src, "chainkit/skills.py", "2"),
          windowed(src, "chainkit/skills.py", "2")[:80])
...
def test_parity_reads_the_seat_map_not_a_constant(reg, lib, book):
    ...
    from chainkit.parity import Report, Outcome
    ...
    check("   and its comment no longer contradicts the docstring",
          ...
          (ROOT / "chainkit" / "parity.py").read_text(encoding="utf-8"))
...
def test_the_chain_can_say_what_it_has_proved(reg, lib, book):
    ...
    tests/last_run.json; each run appends run_history.jsonl; chainkit.us
    ...
    import json
    g = Path(tempfile.mkdtemp())
    (g / "tests").mkdir(); (g / "chainkit").mkdir()
    (g / "chainkit" / "x.py").write_text("# ground\n", encoding="utf-8",
                                         newline="\r\n")
    env = env_for(g, reg, Stub())
    env.ground = g
    ...
    check("   and names the command that would", "test_chainkit.py" in out, out[:90])
    ...
    from chainkit.skills import WRITING_SKILLS, REVIEW_ONLY_SKILLS
...
def test_a_malformed_flag_is_still_read_and_still_stripped(reg, lib, book):
    ...
    from chainkit.pipeline import read_flags, strip_control
...
def test_prune_evicts_undeclared_roots_but_refuses_a_large_one(reg, lib, book):
    ...
    from chainkit.vectors import VectorIndex
...
def test_a_commit_subject_is_the_operators_or_gits_never_the_models(reg, lib, book):
    ...
    chainkit/intent.py has ZERO overlap, exactly like the fabrications. A
...
def test_the_deliberation_renders_as_prose_not_a_column(reg, lib, book):
    ...
    from chainkit.transcript import write as _write
...
def test_the_manifest_reconciles_to_the_disk(reg, lib, book):
    ...
    def write(recs, name="chainkit.us", prose="# trial\n\n"):
        body = prose + "\n\n".join(
    ...
    def honest():
        ...
        for n in seats:
            ...
            out.append({"id": f"chain_{n}", "kind": "agent", "model": seat.model,
                        "can_approve": False,
    ...
    check("main() ASKS the rack rather than defaulting to None",
          "rack_tags()" in (ROOT / "chainkit" / "us.py").read_text(encoding="utf-8"),
    ...
    recs = [dict(r, model="not-a-real-tag:9b") if r["id"] == "chain_router"
            else r for r in honest()]
    write(recs)
    check("a seat declaring the wrong model is found",
          ("us/chain_router", "model") in ids(us.reconcile(g, reg, lib)))

    recs = [dict(r, may_call=[]) if r["id"] == "chain_router" else r
            for r in honest()]
    write(recs)
    check("a seat understating its clearance is found",
          ("us/chain_router", "may_call") in ids(us.reconcile(g, reg, lib)))
    ...
    recs = [dict(r, permission={"edit": {"*": "deny"}})
            if r["id"] == "chain_router" else r for r in honest()]
    write(recs)
    check("a seat cleared for a WRITING skill but declaring edit:deny is found",
          ("us/chain_router", "permission.edit") in ids(us.reconcile(g, reg, lib)))
    ...
    write([{"id": "chain_router", "kind": "agent", "model": "x",
    ...
    check("an OLD-SHAPED permission reports instead of crashing",
          any(f.where == "us/chain_router" for f in old), str(len(old)))
...
def test_a_greeting_never_reaches_the_reader(reg, lib, book):
    ...
    for real in ("what is a good way to read the ground?",
        ...
        from chainkit.skills import REVIEW_ONLY_SKILLS as _RO
    ...
    check("and the test reads the first TWO words, not the first",
          "words[:2]" in (ROOT / "chainkit" / "intent.py")
...
def test_the_deliberation_is_kept_and_never_spoken(reg, lib, book):
    ...
    check("think_to is OPTIONAL, so every other caller is untouched",
          "think_to=None" in (ROOT / "chainkit" / "runtime.py")
          .read_text(encoding="utf-8"))
    ...
    src = (ROOT / "chainkit" / "runtime.py").read_text(encoding="utf-8")
...
def test_the_dedup_keys_on_the_declared_call(reg, lib, book):
    ...
    from chainkit.skills import SkillSpec as _Spec
...
def _handler_src(lib, keyword: str) -> str:
    ...
    import inspect
    from chainkit.skills import _HANDLERS
...
def test_no_skill_is_dead_surface(reg, lib, book):
    ...
    held = os.environ.pop("CHAINKIT_GIT_REMOTE", None)
    try:
        for tool in ("git_pull", "git_push"):
            out = lib.execute(tool, {}, env)
            check(f"{tool} refuses at the remote wall, as prose",
                  out.startswith("Refused") and "CHAINKIT_GIT_REMOTE" in out,
                  out[:80])
    finally:
        if held is not None:
            os.environ["CHAINKIT_GIT_REMOTE"] = held
...
def test_the_citation_check(reg, lib, book):
    ...
    from chainkit import intent
    from chainkit.pipeline import bogus_citations
...
def test_a_skill_cannot_hang_the_repl(reg, lib, book):
    ...
    check("the refusal cites the law and carries the cure",
          "LAW 7" in out and "CHAINKIT_SKILL_TIMEOUT" in out, out[:120])
...
def test_native_tool_calling(reg, lib, book):
    ...
    """
    from chainkit.runtime import calls_to_action_xml
    from chainkit.skills import REVIEW_ONLY_SKILLS as REVIEW_ONLY
...
def test_the_router_is_told_how_not_just_what(reg, lib, book):
    ...
    all of them.
    """
    from chainkit.pipeline import _router_prompt
    from chainkit.skills import ROUTING_DESC_CHARS
...
def test_a_thinking_router_is_never_silent(reg, lib, book):
    ...
    streams.
    """
    from chainkit.runtime import OllamaRuntime, thinking_of, _salvage
...
def test_path_gate(reg, lib, book):
    ...
    """
    import re as _re
    from chainkit.skills import SkillSpec, gate_paths, parse_path_args
    g = Path(tempfile.mkdtemp())
    env = env_for(g, reg, Stub())
...
def test_flags_are_not_speech(reg, lib, book):
    ...
    """
    from chainkit.pipeline import build_prompt, read_flags, strip_control
...
def test_record_and_git():
    ...
    gitstate.commit(g, "chainkit: test")
    st = gitstate.read(g)
    ...
    ctx.steps.append(type(ctx.steps)and __import__("chainkit.context", fromlist=["StepResult"])
...
def main() -> int:
    ...
    test_the_story_and_the_hands(reg, lib, book)
    test_the_headless_door(reg, lib, book)
    test_the_ground_flag(reg, lib, book)
    test_the_hands_close_says_what_it_did(reg, lib, book)
