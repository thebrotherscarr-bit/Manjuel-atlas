...
(Corrected 2026-09-04 to match `law/pen/links.py`, which is what actually
runs.) Each link's `hash` is `sha256(prev + canonical JSON of the link body)`;
each law FILE is fingerprinted separately by `sha256` over its exact bytes as
they lie on disk, and that fingerprint is carried in the link's payload. Walk
the chain: link 1's `prev` is sixty-four zeros and its payload cites the
covenant; every later `prev` equals the prior link's `hash`. A broken link
names itself — the chain refuses to be history that lies. The
`--- ENTRY n ---` block format this section once described was never built.

---

## THE LAW — base text (v1.0, adopted 2026-08-24)

*Ruled 2026-09-04: these ten are **THE ESTATE LAWS** — for the seats. A
bare `LAW n` anywhere in the house means ESTATE LAW n. They are also held
in `law/ESTATE_LAWS.md`, so the chain fingerprints them. The operator's
laws for the hands are **THE SITTING LAWS**, `law/SITTING_LAWS.md`, cited
as SITTING LAW n.*
...
- **Chain:** `law\chain.jsonl` — hash-linked links, wraps at
  forty by arithmetic (the wrap IS the token), Merkle-provable.
- **Library:** `law\*.md` — every law is a standard markdown
  file; every link points at one and carries its sha256 fingerprint.
- **Tool:** `python law\law.py direct|counsel|rule|refuse|verify|status`
...
2026-09-04: the folder flattened to `law\` (SITTING LAW 4). DIRECT on
ESTATE_LAWS.md (the ten, under seal) and SITTING_LAWS.md (the operator's
four) — sealed by the operator
with `law.py direct`; if `law.py status` does not show them, they are
written but not yet laid.
